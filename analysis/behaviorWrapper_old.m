...
%% to run when updating thresholds with just one mouse

if length(mouseList) == 1
    
    currMouse = mouseList{1};
    currThresh = threshold;
    
    % load the current thresholds and update them
    load('..\thresholds2use.mat');
    mouseList{end+1} = currMouse;
    threshold(end+1,:) = currThresh;
    
    % save a copy of the old file
    fn = ['..\thresholds2use_' datestr(now,'yymmdd') '.mat'];
    copyfile('..\thresholds2use.mat',fn);
    
    % save out the new file
    save('..\thresholds2use.mat','mouseList','threshold');
    keyboard
end
...
if 1 == 2
    ...
    for i = 1:size(threshB,1)
        threshold(list(i),1) = threshB{i}(1);
        %threshold(mouseList(i),2) = threshB{i,2}(end);
    end
